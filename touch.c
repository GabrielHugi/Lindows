...
THERE ARE MANY OPPORTUNITIES TO CUT DOWN ON GARBAGE REPEATED CODE. LATER MAKE SURE
TO REMEMBER TO CUT DOWN ON IT AND TO STANDARDIZE SOME OF THE CODE
ALSO CHECK IF ANY FLAGS ARE MISSING AND THEN CHECK IF EVERYTHINK WORKS
+ 
MAKE IT SO WITHOUT -H WHEN SOMEONE TRIES TO ALTER LINK DATES IT ACTUALLY ALTERS WHAT IT POINTS TO
...
int main (int argc, char** argv) {
    ...
    // please remove this stupid ass for loop at some point me
    for (int i = 1; i < argc; i++) {
            // help
            if (argc == 1) {
                printf("help");
            ...
            if (modifiers.h == 1) {
                ...
                if (modifiers.t == 1) {
                    ...
                    if ((modifiers.a == 0 && modifiers.m == 0) || (modifiers.a == 1 && modifiers.m == 1)) {
                        sendtime.actime = epochconv;
                        sendtime.modtime = epochconv;
                    }
                    if (modifiers.a == 1 && modifiers.m == 0) {
                        sendtime.actime = epochconv;
                        sendtime.modtime = tmm.st_mtime;
                    }
                    if (modifiers.m == 1 && modifiers.a == 0) {
                        sendtime.actime = tmm.st_atime;
                        sendtime.modtime = epochconv;
                    }
                    ...
                    utime(argv[pos], &newtime);
                }
            if (modifiers.t == 1 && modifiers.h != 1) {
                int pos = 1+modnum;
                int size = strlen(argv[pos]);
                short year = 2000, month = 0, day = 0, hour = 0, minute = 0, second = 0;
                time_t epochconv;
